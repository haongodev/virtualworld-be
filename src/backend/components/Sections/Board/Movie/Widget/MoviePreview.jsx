...
export default function MoviePreview({ values, isValid, isSubmitting, isOpen, onClose, onSubmit }) {
  ...
  return (
    <DialogAnimate fullScreen open={isOpen} onClose={onClose} variants={undefined} sx={undefined}>
      ...
      {hasContent ? (
        ...
        <EmptyContent title="Empty content" description={undefined} img={undefined} />
      )}
    </DialogAnimate>
  );
}
...
function PreviewHero({ title, cover }) {
  return (
    <Box sx={{ position: 'relative' }}>
      ...
      />
      <Image alt="cover" src={cover} ratio="16/9" sx={undefined} />
    </Box>
  );
}
